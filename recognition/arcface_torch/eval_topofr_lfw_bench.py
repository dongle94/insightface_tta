...
if __name__ == '__main__':
    ...
    log_file = os.path.join(os.path.dirname(__file__), './exp/benchmark_asian_r200.txt')
    # 콘솔과 파일에 동시 출력하도록 설정
    ...
    model = get_topofr_model(args.network)
    # weight = torch.load(model_path)
    # model.load_state_dict(weight)

    # 단일 모델을 테스트 할 때는 아래줄과 for loop 을 주석처리하고 인덴트 땡기고, 위의 cfg, resnet, weight 주석해제 하여 사용
    # model_paths = sorted([os.path.join(args.model_dir, f) for f in os.listdir(args.model_dir) if f.endswith('.pt')])
    # for model_path in model_paths:
    # model = torch.load(model_path)
    # model = torch.nn.DataParallel(resnet)     # 멀티 GPU 일 때만 사용
    model.eval().to(device)
    
    print("** --------------------------------- **")
    # print(f"** Model Architecture loaded: {args.model_config}")
    # print(f"** Model Weight loaded: {model_path}")

    path_list, issame_list = get_paths(dataset_dir)
    embeddings_dict = get_embeddings_from_pathlist(path_list, batch_size=batch_size)
