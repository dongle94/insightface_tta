...
if __name__ == '__main__':
    ...
    episodic = [True]
    image_paths = [  
        '/data/dataset/Face/benchmark/cplfw'
    ] 
    ...
    for dataset_dir in image_paths:
        ...
        for name in names:
            ...
            for tbs in batch_sizes:
                lr = (0.00025 / 64) * tbs * 2 if tbs < 32 else 0.00025
                for epi in episodic:
                    for ss in steps:
                        # get model
                        o_model, t_model, param = get_saented_model(name, saent_steps=ss, episodic=epi)
